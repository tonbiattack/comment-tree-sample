...
func (r *Repository) GetCommentSubtree(ctx context.Context, commentID int64) ([]Comment, error) {
	var path string
	if err := r.db.QueryRowContext(ctx, `SELECT path FROM comments WHERE id = ?`, commentID).Scan(&path); err != nil {
		return nil, fmt.Errorf("get subtree path: %w", err)
	}
	...
	return r.queryComments(ctx, query, path)
}
