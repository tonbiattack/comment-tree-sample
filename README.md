MySQL 8 を Docker で起動できる最小構成を追加しています。ホスト側ポートは固定せず、Docker に空きポートを自動割り当てさせています。

3方式の比較資料は `docs/tree-model-comparison.md` を参照してください。

現代の実務で最初に選びやすいのは `隣接リスト` です。`Materialized Path` は読み取りを少し強くしたいときの現実的な拡張案、`閉包テーブル` は祖先子孫検索を強く最適化したい場合の選択肢です。
...
このスクリプトは MySQL を起動し、実際に割り当てられた公開ポートを表示します。
...
```text
Host: 127.0.0.1
Port: docker compose port mysql 3306 で確認
...
`test/testdb` は `docker compose port mysql 3306` を自動検出するので、VS Code のテスト実行からも環境変数なしで動作します。
...
business unanswered root threads for post 2:
- root_id=5 direct=0 descendants=0
business posts by recent activity:
- post_id=2 title=PostB total=1 latest=2026-01-01T11:00:00Z
- post_id=1 title=PostA total=4 latest=2026-01-01T10:04:00Z
- post_id=3 title=PostC total=0 latest=-
```
